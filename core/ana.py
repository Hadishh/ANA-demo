...
class ChatBot:
    ...
    def answer(self, message, dictionnaire):
        message_text = message["text"]
        inquiry_type = self.functionality_identifier.exctract_functionality(message_text)
        ...
        if dictionnaire == '':
            ...
            if "functionality query" in inquiry_type:
                ...
                intent_type = self.intent_classifier.extract_intent_type(message_text)
                if "factual question" in intent_type:
                    #WEIRD
                    factuality = self.factuality_separator.factuality_separate(message_text)
                    if 'not factual question' in factuality:
                        response = self.__non_factual_answer(message_text)
                    # factual question
                    else:
                        response = self.__factual_answer(message_text)
                # yes no questions
                elif "yes/no question" in intent_type:
                    response = self.__yes_no_answer(message_text)
                # Orders
                elif 'direct order' in intent_type or 'indirect order' in intent_type:
                    response = self.__order_answer(message_text)

                elif "greeting" in intent_type:
                    response = self.__greeting(message_text)
                
                elif "statement" in intent_type:
                    response = self.__order_answer(message_text)
                
                elif "apology" in intent_type or "feedback" in intent_type:
                    response = self.__other_inquiry(message_text)
